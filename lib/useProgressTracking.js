...
export function useProgressTracking(videoRef, metadata, enabled) {
  ...
  useEffect(() => {
    ...
    const handleTimeUpdate = () => {
      ...
      // Use sourceDuration as the FIXED denominator when available;
      // fallback to video.duration + offset (old growing behavior).
      const sourceDur = metadataRef.current?.sourceDuration;
      const realDuration = sourceDur && sourceDur > 0
        ? sourceDur
        : (video.duration || 0) + off;
