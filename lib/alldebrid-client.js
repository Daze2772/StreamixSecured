...
export async function resolveAllDebrid({ imdbId, mediaType, season, episode, adApiKey, cometManifestUrl }) {
  ...
  try {
    ...
    // Step 5: Wait and get status (increased wait time for larger files)
    await sleep(2000);
    
    let status;
    for (let i = 0; i < 15; i++) {
      status = await getMagnetStatusAD(magnet.id, adApiKey);
      console.log(`[AD Client] Attempt ${i + 1}/15: status=${status.status}`);
      
      if (status.status === 'Ready') break;
      if (i < 14) await sleep(3000); // Wait 3 seconds between checks
    }

    if (status.status !== 'Ready') {
      throw new Error('AllDebrid is still preparing the file (waited 45+ seconds). This torrent may not be cached. Try RD Premium or another server.');
    }
