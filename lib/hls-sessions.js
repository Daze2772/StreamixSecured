...
function chooseFfmpegArgs(url, codecs, outDir) {
  ...
  // HLS muxer.
  // We deliberately use playlist_type `event` (writes incrementally as
  // ffmpeg encodes) rather than `vod` (which sometimes defers the playlist
  // write until ffmpeg finishes). `event` is technically a "live" playlist
  // type but with `hls_list_size 0` we keep ALL segments, so seeking still
  // works from the start. hls.js handles event playlists just fine.
  args.push(
    '-f', 'hls',
    '-hls_time', '4',
    '-hls_list_size', '0',
